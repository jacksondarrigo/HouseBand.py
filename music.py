...
ffmpegopts = {
    'before_options': '-reconnect 1 -nostdin',
    'options': '-vn'
}
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        return cls(discord.FFmpegPCMAudio(source, before_options=ffmpegopts['before_options'], options=ffmpegopts['options']), data=data, requester=ctx.author)
    ...
    async def regather_stream(cls, data, *, loop):
        ...
        return cls(discord.FFmpegPCMAudio(data['url'], before_options=ffmpegopts['before_options'], options=ffmpegopts['options']), data=data, requester=requester)
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if not vc:
            if ctx.author == ctx.guild.owner:
                ...
                channel = next((voice_channel for voice_channel in voice_channels if voice_channel.name == search.split()[0]), None)
